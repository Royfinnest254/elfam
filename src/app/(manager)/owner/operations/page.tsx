  ...
  return (
    <div className="space-y-8 font-sans text-[#091E42] pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-6 bg-white border border-[#DFE1E6] p-6 rounded-[24px] space-y-4">
          <div className="flex items-center gap-2 border-b border-[#DFE1E6] pb-3">
            <Tractor className="h-5 w-5 text-[#5E6C84]" />
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-[#DFE1E6] font-semibold text-[#091E42]">
                {fields.map((field) => {
                  ...
                  return (
                    <tr key={field._id} className="hover:bg-[#F4F5F7]/30">
                      ...
                      <td className="py-3 uppercase text-[10px]">
                        <span className={`status-badge ${
                          ...
                          field.crop === "barley" ? "bg-[#E3FCEF] text-[#006644] border-[#E3FCEF]" :
                          "bg-[#DFE1E6] text-[#5E6C84] border-[#DFE1E6]"
                        }`}>
                          {field.crop}
                        </span>
                      </td>
                      <td className="py-3 text-[#5E6C84] uppercase tracking-wide text-[10px]">
                        <span className={`status-badge ${
                          stage === "ready for harvest" ? "bg-[#FFEBE6] text-[#BF2600] border-[#FFEBE6]" :
                          stage === "flowering" ? "bg-[#FFF0B3] text-[#172B4D] border-[#FFF0B3]" :
                          stage === "vegetative" ? "bg-[#E3FCEF] text-[#006644] border-[#E3FCEF]" :
                          stage === "fallow" ? "bg-[#F4F5F7] text-[#7A869A] border-[#DFE1E6]" :
          ...
          <div className="pt-4 border-t border-[#DFE1E6] text-xs font-bold text-[#5E6C84]">
            Total crop acreage: <span className="text-[#091E42] font-mono">{activeCropAcreage} acres</span>.
          </div>
        </section>

        {/* Right Column: Contracts & Deliveries */}
        <section className="lg:col-span-6 bg-white border border-[#DFE1E6] p-6 rounded-[24px] space-y-6">
          <div className="flex justify-between items-center border-b border-[#DFE1E6] pb-3">
            <div className="flex items-center gap-2">
              <ClipboardList className="h-5 w-5 text-[#5E6C84]" />
              <h3 className="text-sm font-black uppercase tracking-wider text-[#091E42]">
          ...
          {activeBarleyContract ? (
            <div className="space-y-6">
              ...
              <div className="bg-[#F4F5F7] border border-[#DFE1E6] p-5 rounded-xl space-y-4 text-xs font-semibold text-[#5E6C84]">
                <div className="flex justify-between items-center">
                  <span className="text-[#091E42] font-bold text-sm">EABL Contract: Barley</span>
                  <span className="status-badge status-ok text-[9px] uppercase px-2 py-0.5">ACTIVE</span>
                </div>
                <div className="grid grid-cols-3 gap-2 pt-2 border-t border-[#DFE1E6]">
                  <div>
                    <span className="text-[9px] uppercase tracking-wider text-[#7A869A] block">Unit Price</span>
                    <span className="text-[#091E42] font-bold font-mono">KES {activeBarleyContract.pricePerBag}</span>
                  </div>
                  <div>
                    <span className="text-[9px] uppercase tracking-wider text-[#7A869A] block">Delivered</span>
                    <span className="text-[#006644] font-bold font-mono">{totalDeliveredBags} Bags</span>
                  </div>
                  <div>
                    <span className="text-[9px] uppercase tracking-wider text-[#7A869A] block">Remaining</span>
                    <span className="text-[#BF2600] font-bold font-mono">{bagsRemaining} Bags</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <span className="text-[10px] font-black text-[#5E6C84] uppercase tracking-wider block">Deliveries Log</span>
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-[#DFE1E6] font-semibold text-[#091E42]">
                      {barleyDeliveries.length === 0 ? (
                        <tr>
                          <td colSpan={3} className="py-4 text-center text-xs italic text-[#5E6C84]">
                        ...
                        barleyDeliveries.map((d) => (
                          <tr key={d._id}>
                            <td className="py-2.5 font-mono text-[11px] text-[#5E6C84]">
                              {new Date(d.date).toLocaleDateString("en-GB")}
                            </td>
                            <td className="py-2.5 font-mono text-right text-[#091E42]">{d.bags} Bags</td>
            ...
            <p className="text-xs text-[#5E6C84] italic font-semibold">No active Barley contracts found.</p>
          )}
        </section>
      </div>
      ...
      {logFormOpen && activeBarleyContract && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#091E42]/60 p-4 animate-in fade-in duration-200">
          <div className="bg-white border border-[#DFE1E6] rounded-[24px] max-w-[420px] w-full p-6 space-y-6 relative shadow-elevated">
            <button
              type="button"
              onClick={() => setLogFormOpen(false)}
              className="absolute top-5 right-5 p-2 text-[#5E6C84] hover:text-[#091E42] hover:bg-[#F4F5F7] rounded-xl transition-all cursor-pointer"
            >
              <X className="h-5 w-5" />
            </button>

            <div>
              <h2 className="font-display text-lg font-black uppercase tracking-tight text-[#091E42]">
                Log Contract Delivery
              </h2>
              <span className="text-[10px] uppercase font-bold text-[#5E6C84]">
                EABL Barley contract &middot; {activeBarleyContract.season}
              </span>
            </div>
            ...
            {error && (
              <div className="bg-[#FFEBE6] border border-[#FFD2C7] text-[#BF2600] text-xs font-semibold p-4 rounded-xl">
                {error}
              </div>
            )}
