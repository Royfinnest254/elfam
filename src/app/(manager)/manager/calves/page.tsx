...
export default function ManagerCalvesPage() {
  ...
  if (calves === undefined) {
    return <div className="text-xs text-[#5E6C84] uppercase font-black tracking-widest p-8 font-sans">Loading calves register...</div>;
  }
  ...
  return (
    <div className="space-y-8 font-sans text-[#091E42] pb-12">
      <header className="border-b border-[#DFE1E6] pb-6">
        ...
        <p className="text-xs text-[#5E6C84] font-semibold mt-1 uppercase tracking-wider">Registry of young stock heifers and bull calves under weaning age</p>
      </header>

      {/* Calves list */}
      <div className="system-card p-6 space-y-4">
        {calves.length === 0 ? (
          <p className="text-xs text-[#5E6C84] italic font-semibold">No young calves registered in database.</p>
        ) : (
          <div className="overflow-x-auto table-scroll custom-scrollbar">
            <table className="w-full text-left text-xs divide-y divide-[#DFE1E6]">
              ...
              <tbody className="divide-y divide-[#DFE1E6] font-medium text-[#091E42]">
                {calves.map((calf: any) => (
                  <tr key={calf._id} className="hover:bg-[#F4F5F7]/50 transition-colors">
                    ...
                    <td className="p-4 font-mono text-[#5E6C84]">{new Date(calf.dateOfBirth).toLocaleDateString("en-GB")}</td>
                    <td className="p-4 font-mono">{calf.currentWeight} Kg</td>
                    <td className="p-4 font-mono text-[#5E6C84]">{calf.sireInfo}</td>
                    <td className="p-4">
