...
export default function ManagerLivestockPage() {
  ...
  const deleteLivestockMutation = useMutation(api.records.deleteLivestock);
  ...
  const handleDeleteLivestock = async (id: any) => {
    if (!confirm("Are you sure you want to delete this animal record?")) return;
    try {
      await deleteLivestockMutation({ id });
      setStatusMsg("VERIFIED: Livestock record deleted.");
    } catch (err: any) {
      console.error(err);
