...
const ownerNav = [
  { name: "Executive Overview",  href: "/owner",            icon: Home },
  { name: "Agribusiness Herd",  href: "/owner/herd",       icon: Layers },
  { name: "Cereal Contracts",   href: "/owner/operations",   icon: Map },
  { name: "Gross Revenue",      href: "/owner/finances",     icon: Tractor },
  { name: "Staff Directory",    href: "/owner/people",       icon: Users },
  { name: "Print Reports",      href: "/owner/reports",      icon: FileText },
];
...
function NavItem({
  ...
  const isActive =
    item.href === "/manager" || item.href === "/worker" || item.href === "/supervisor" || item.href === "/owner"
      ? pathname === item.href
  ...
  return (
    <Link
      ...
      className={`
        group flex items-center gap-3 py-3 px-6 rounded-none text-[13px] font-sans font-medium
        transition-colors duration-150 border-0 w-full
        ${
          isActive
            ? "bg-cream text-moss"
            : "text-cream/80 hover:bg-moss-2 hover:text-cream"
        }
      `}
    >
      <Icon
        className={`h-4 w-4 shrink-0 transition-colors ${
          isActive ? "text-moss" : "text-cream/60 group-hover:text-cream"
        }`}
      />
...
export default function Sidebar() {
  ...
  if (role === "supervisor") navItems = ownerNav;
    ...
    if (user?.imageUrl) {
      return (
        ...
        <img
          src={user.imageUrl}
          alt={user?.name ?? "PFP"}
          className="w-7 h-7 rounded-none object-cover shrink-0 border border-moss-2"
        />
      );
    }
    return (
  ...
  return (
    <>
      ...
      {/* ── Desktop sidebar ─────────────────────────── */}
      <aside
        className="hidden md:flex flex-col w-[240px] shrink-0 h-screen sticky top-0 z-20 bg-moss text-cream border-r border-moss-2"
      >
        {/* Logo block */}
        <div className="px-5 py-4 border-b border-moss-2">
          <div className="flex items-center gap-2 mb-2">
            <Wordmark size={22} tone="cream" />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-mono uppercase tracking-[0.1em] text-cream/60">
              {role} portal
            </span>
          </div>
        </div>
        ...
        {/* Nav */}
        <nav className="flex-1 overflow-y-auto px-0 py-4 space-y-0.5 custom-scrollbar">
          {navItems.map((item) => (
            <NavItem key={item.href} item={item} pathname={pathname} />
          ))}

          <div className="pt-2 mt-2 border-t border-moss-2">
            <NavItem
              item={{ name: "Settings", href: settingsHref, icon: Settings }}
              pathname={pathname}
            />
        ...
        {/* User footer */}
        <div className="px-3 pb-4 border-t border-moss-2 pt-3 space-y-3">
          <div className="flex items-center gap-3 px-2 py-2 rounded-none border border-moss-2 bg-moss-2">
            {renderAvatar()}
            <div className="min-w-0 flex-1">
              <p className="text-[12px] font-bold text-cream truncate leading-tight">
                {user?.name ?? "User"}
              </p>
              <p className="text-[11px] text-cream/70 truncate leading-tight">
                {user?.email ?? "loading..."}
              </p>
            </div>
            <Link
              href={settingsHref}
              title="Settings"
              className="p-1.5 rounded-none hover:bg-moss text-cream hover:text-cream transition-colors shrink-0"
            >
          ...
          <button
            ...
            id="signout-btn"
            onClick={handleSignOut}
            className="w-full flex items-center justify-center gap-2 h-9 rounded-none border border-moss-2 text-cream text-[12px] font-medium hover:bg-moss-2 transition-colors"
          >
